...
/**
 * Class JL_Bruno_FreeInstallerScript
 */
class JL_Bruno_FreeInstallerScript
{
    /** @var string */
        ...
        try {
            if (!class_exists("Gantry5\Loader")) {
                throw new RuntimeException(
                    sprintf(
                        "Please install Gantry 5 Framework before installing %s template!",
                        $name
                    )
                );
            }
            ...
            if (
                !method_exists($gantry, "isCompatible") ||
                !$gantry->isCompatible($this->requiredGantryVersion)
            ) {
                throw new \RuntimeException(
                    sprintf(

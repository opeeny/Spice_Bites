...
$input = '<input type="text" name="q" id="mod-finder-searchword' . $module->id . '" class="js-finder-search-query jl-search-input" value="' . htmlspecialchars($app->getInput()->get('q', '', 'string'), ENT_COMPAT, 'UTF-8') . '"'
    . ' placeholder="' . Text::_('MOD_FINDER_SEARCH_VALUE') . '">';

$showLabel  = $params->get('show_label', 1);
$labelClass = (!$showLabel ? 'visually-hidden ' : '') . 'jl-hidden finder';
...
if ($params->get('show_button', 0)) {
    $output .= $label;
    $output .= '<div class="mod-finder__search">';
    $output .= $input;
    $output .= '<button jl-search-icon class="jl-search-icon-flip jl-icon jl-search-icon" type="submit" aria-label="' . Text::_('JSEARCH_FILTER_SUBMIT') . '"></button>';
    $output .= '</div>';
} else {
    $output .= '<span jl-search-icon class="jl-position-z-index"></span>';
    $output .= $label;
    $output .= $input;
}
...
<search>
    <form class="mod-finder js-finder-searchform jl-search jl-search-default jl-width-1-1" action="<?php echo Route::_($route); ?>" method="get" aria-label="search">
        <?php echo $output; ?>
